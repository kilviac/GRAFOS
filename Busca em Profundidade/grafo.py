...
class Grafo:
    ...
    def dfs(self, x):
        arestas = deepcopy(self.A)
        vertices = deepcopy(self.N)
        v = vertices.index(x)
        arestas_inver = [] # lista arestas com os vértices invertidos
        pilha = []
        ...
        while True:
            ...
            for j in adj: # busca por um vizinho não visitado
                if visitados[vertices.index(j)] == False:
                    novoVert = vertices.index(j)
                    achou = True
                    break

            if achou:
                v = novoVert # pega o próximo vértice
            else:  # se todos os vizinhos estão visitados ou não existem vizinhos
                for l in arestas_inver: # adicionar as arestas de retorno
                    m = l.split('-')
                    if (vertices[novoVert] == m[0] or vertices[novoVert] == m[1]) and (vertices[v] == m[0] or vertices[v] == m[1]): # vê se existe a aresta na lista de arestas invertidas
                            listArestasRetorno.append(l) # se existir, adiciona na lista de retorno
                            break
                    elif (vertices[v] == m[0]): # quando todas as arestas dos filhos já foram adicionadas, faltando a do vértice com o seu pai
                        listArestasRetorno.append(l)
                        break
