...
class Grafo:
    ...
    def dijkstra(self, inicial, final, cargaI, pontos):
        ...
        for i in range(len(vertices)):
            if vertices[i] == inicial: u = i
            if vertices[i] == final: v = i
            if vertices[i] in pontos: lista_de_recarga.append(i)
        ...
        for i in range(len(vertices)):
            if 1 not in arestas[i] and i != v: phi[i] = 1
        for i in lista_de_recarga: cargas[i] = 5
        ...
        while True:
            ...
            for i in range(len(lista)):
                if lista[i] > 0 and beta[i] > beta[w] + lista[i] and cargas[w] - 1 >= 0 or (
                        lista[i] > 0 and cargas[i] == 0 and cargas[w] - 1 >= 0):
                    beta[i] = beta[w] + lista[i]
                    if cargas[i] != 5: cargas[i] = cargas[w] - 1
                    pi[i] = w
            ...
            for i in range(len(beta)):
                if beta[i] < menor and phi[i] == 0 and cargas[i] > 0 and (1 in arestas[i] and i != v or i == v):
                    menor = beta[i]
                    aux = i

            if aux == v or menor == 999: break
        ...
        for i in range(len(caminho)):
            if i == len(caminho) - 1:
               break
                ...
                concatena = var_aux[i] + '-' + var_aux[i+1]
                lista_caminho.append(concatena)
